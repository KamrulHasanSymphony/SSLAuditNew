...
namespace Shampan.Core.Interfaces.Services.Deshboard
{
    public interface IDeshboardService : IBaseService<AuditMaster>
    {
        ...
        ResultModel<DeshboardSettings> DeshboardSettingUpdate(DeshboardSettings model);
        ResultModel<DeshboardSettings> DeshboardSettingInsert(DeshboardSettings model);
        ...
        ResultModel<List<DeshboardSettings>> DeshboardSettingGetIndexData(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);
        ResultModel<int> DeshboardSettingGetIndexDataCount(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);
        int DeshboardSettingGetCount(string tableName, string fieldName, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);



        ResultModel<List<PrePaymentMaster>> NonFinancialGetAll(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);
        ResultModel<List<DeshboardSettings>> DeshboardSettingGetAll(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);
        ...
        bool AuditTeamMemberGetAll(string userName,string userId);
        ResultModel<PrepaymentReview> PrepaymentReviewInsert(PrepaymentReview model);
        
		ResultModel<List<AuditMaster>> GetTotalBranchWithAuditCount(AuditMaster model);
		ResultModel<List<AuditIssue>> GetIssueCategoryData(AuditIssue model);
