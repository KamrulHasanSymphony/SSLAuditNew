...
namespace Shampan.Core.Interfaces.Services.Audit
{
    public interface IAuditIssueUserService : IBaseService<AuditIssueUser>
    {
        ...
        List<AuditIssueUser> GetAuditIssueUsersList(string userId,int? id);
        ResultModel<AuditIssueUser> AudiIssueUserUpdateById(AuditIssueUser model);
        ResultModel<List<AuditIssueUser>> GetAuditIssueUserById(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null);
    }
}
