    ...
    public class AuditMasterService : IAuditMasterService
    {
        ...
        public ResultModel<AuditMaster> Insert(AuditMaster model)
        {
            ...
            try
            {
                ...
                //foreach (var detail in model.BKAuditTemplateDetails)
                //{
                //    detail.AuditId = model.Id;
                //    detail.BKAuditOfficeTypeId = detail.BKAuditOfficeTypeId;
                //    detail.BKAuditTemlateMasterId = detail.BKAuditTemlateMasterId;
                ...
                //    detail.BKCheckListItemId = detail.BKCheckListItemId;
                //    detail.IsFieldType = detail.IsFieldType;
                //    detail.Status = detail.Status;
                //    BKAuditTemplateDetail = context.Repositories.AuditMasterRepository.AuditTemplateDetails(detail);
        ...
        public ResultModel<AuditMaster> Update(AuditMaster model)
        {
            ...
            try
            {
                ...
                CheckIssueStatus = context.Repositories.AuditMasterRepository.CheckIssueStatus("A_AuditIssues", new[] { "AuditId" }, new[] { model.Id.ToString() });
                if (CheckIssueStatus)
                {
                    return new ResultModel<AuditMaster>()
                    {
                        Status = Status.Fail,
                        Message = MessageModel.IssueAlready,
                ...
                if (master.Id > 0)
                {
                    ...
                    int auditUsersRecondCount = context.Repositories.AuditMasterRepository.DetailsDelete(TableName.AuditUsers,
            new[] { "AuditId", "TeamId" }, new[] { model.Id.ToString(), model.TeamValue.ToString() });



                    if (auditUsersRecondCount > 0)
                    {
                        foreach (var User in master.AuditUsers)
                        {

                            User.Audit.CreatedBy = master.Audit.LastUpdateBy;
                            User.AuditId = master.Id;
                            User.Audit.CreatedOn = DateTime.Now;
                            User.Audit.CreatedFrom = "";

                            AuditUser auditUser = context.Repositories.AuditUserRepository.Insert(User);
                ...
                if (master.CompanyCode.ToUpper() == "BRAC")
                {

                    if (master.Id > 0)
                    {

                        int recordCount = context.Repositories.AuditMasterRepository.DetailsDelete(
                            TableName.AuditTemplateDetails,
                            new[] { "AuditId" }, new[] { master.Id.ToString() });
        ...
        public ResultModel<AuditMaster> BranchFeedbackCompleted(AuditMaster model)
        {
            using (IUnitOfWorkAdapter context = _unitOfWork.Create())
            {
                ...
                try
                {
                    AuditMaster records = context.Repositories.AuditMasterRepository.BranchFeedbackCompleted(model);
        ...
        public ResultModel<AuditMaster> FeedbackComplete(AuditMaster model)
        {
            using (IUnitOfWorkAdapter context = _unitOfWork.Create())
            {
                ...
                bool CheckListItemsStatus = false;

                try
                {

                    CheckListItemsStatus = context.Repositories.AuditMasterRepository.CheckListItemsStatus("", new[] { "" }, new[] { model.Id.ToString() });
                    ...
                    if (CheckListItemsStatus)
                    {
                        ...
                        {
                            Status = Status.Fail,
                            Message = MessageModel.ListItemsNotComplete,
        ...
        public int AuditNotificationUpdate(string notification, int? id)
        {
            ...
            try
            {
                int saveId = context.Repositories.AuditMasterRepository.AuditNotificationUpdate(notification, id);
                context.SaveChanges();
                return saveId;
            }
            catch (Exception e)
