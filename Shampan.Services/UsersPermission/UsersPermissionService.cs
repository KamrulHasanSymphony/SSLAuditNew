...
namespace Shampan.Services.UsersPermission
{
	public class UsersPermissionService : IUsersPermissionService
	{
        ...
        public ResultModel<UserProfile> UserProfileActivate(UserProfile model)
        {
            using (var context = _unitOfWork.Create())
            {
                try
                {
                    UserProfile master = context.Repositories.UsersPermissionRepository.UserProfileActivate(model);
                    context.SaveChanges();
