    ...
    public class AuditBranchFeedbackController : Controller
    {
        ...
        public ActionResult CreateEdit(AuditBranchFeedback master)
        {
            ...
            try
            {
                ...
                if (!isBranch)
                {
                    UserData = _auditMasterService.GetAuditIssueUsers(master.BranchEmailIssueId);
                ...
                else
                {
                    ...
                    UserData = _auditMasterService.GetAuditUsers(master.AuditId);
                ...
                #region Notification
                var currentUrlbf = HttpContext.Request.GetDisplayUrl();
                string[] partsbf = new string[] { "", "" };
                partsbf = currentUrlbf.TrimStart('/').Split('/');
                string urlhttpbf = partsbf[0];
                string HostUrlbf = partsbf[2];
                string url = urlhttpbf + "//" + HostUrlbf + "/Audit/Edit/" + result.Data.AuditId + "?edit=Branchfeedback&common=branchFeedbackNotification " + result.Data.Id;
                //notifiaction = SetNotificationValueForInsert(url, master.Heading, result.Data.AuditId, result.Data.Id, user.UserName, "BranchFeedback");
                //_notificationService.Insert(notifiaction);

                ResultModel<Notifications> resultNotifi = _notificationService.InsertNotification(url, master.Heading, result.Data.AuditId, result.Data.Id, user.UserName, "BranchFeedback", teamMembers.Data);
        ...
        [HttpPost]
        public ActionResult BranchSendEmail(AuditBranchFeedback master)
        {
            ResultModel<AuditBranchFeedback> result = new ResultModel<AuditBranchFeedback>();
            try
            {
        ...
        public ActionResult<IList<AuditBranchFeedback>> _index(int? id)
        {
            try
            {
                ...
                UserBranch singleUser = Users.Data.FirstOrDefault();
                List<AuditIssueUser> userlist = _auditIssueUserService.GetAuditIssueUsersList(singleUser.UserId, id);
                index.AuditIssueUsers = userlist;
                ...
                ResultModel<List<AuditBranchFeedback>> indexData = _auditBranchFeedbackService.GetIndexData(index, conditionalFields, conditionalValue);
                ...
                ResultModel<int> indexDataCount = _auditBranchFeedbackService.GetIndexDataCount(index, conditionalFields, conditionalValue);
        ...
        public async Task<IActionResult> BranchPreviewIndex(int id)
        {
            AuditBranchFeedback abf = new AuditBranchFeedback() { Id = id };
            return View(abf);
        }
        [HttpPost]
        public ActionResult<IList<AuditBranchFeedback>> _branchPreviewIndex(int? id)
        {
            try
            {
                IndexModel index = new IndexModel();
                ...
                ResultModel<List<AuditBranchFeedback>> indexData = _auditBranchFeedbackService.GetIndexDataFeedbackPreview(index, conditionalFields, conditionalValue);
