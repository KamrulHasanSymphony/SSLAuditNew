...
namespace SSLAudit.Controllers
{
	...
	[Authorize]
	public class BKAuditCategorieController : Controller
    {
        private readonly ApplicationDbContext _applicationDb;
        private readonly IAdvancesService _advancesService;
        private readonly ICategoryService _categoryService;

        public BKAuditCategorieController(ApplicationDbContext applicationDb, ITeamsService teamsService,
			IAdvancesService advancesService, ICategoryService categoryService)
        {
		...
		public IActionResult _index()
		{
			try
			{
				...
				ResultModel<List<Category>> indexData =
                    _categoryService.GetIndexData(index, conditionalFields, conditionalValue);

				ResultModel<int> indexDataCount =
                _categoryService.GetIndexDataCount(index, conditionalFields, conditionalValue);

				int result = _categoryService.GetCount(TableName.Category, "Id", new[] { "Advances.createdBy", }, new[] { userName });
			...
			catch (Exception e)
			{
				...
				return Ok(new { Data = new List<Category>(), draw = "", recordsTotal = 0, recordsFiltered = 0 });
			}
        ...
        public ActionResult CreateEdit(Category master)
        {
            ResultModel<Category> result = new ResultModel<Category>();
            try
            {
                ...
                if (master.Operation == "update")
                {
                    ...
                    result = _categoryService.Update(master);
                ...
                else
                {
                    ...
                    result = _categoryService.Insert(master);
		...
		public ActionResult Edit(int id)
		{
			try
            {
                ResultModel<List<Category>> result =
                    _categoryService.GetAll(new[] { "Id" }, new[] { id.ToString() });

                Category category = result.Data.FirstOrDefault();
                category.Operation = "update";
                category.Id = id;

                return View("CreateEdit", category);
            }
            catch (Exception e)
            {
                e.LogAsync(ControllerContext.HttpContext);
