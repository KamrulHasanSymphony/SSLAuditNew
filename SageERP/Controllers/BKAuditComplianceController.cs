...
namespace SSLAudit.Controllers
{
	...
	[Authorize]
	public class BKAuditComplianceController : Controller
    {
        private readonly ApplicationDbContext _applicationDb;
        private readonly IAdvancesService _advancesService;
        private readonly IBKAuditComplianceService _BKAuditComplianceService;

        public BKAuditComplianceController(ApplicationDbContext applicationDb, ITeamsService teamsService,
		IAdvancesService advancesService, IBKAuditComplianceService BKAuditComplianceService

            )
        {
            _applicationDb = applicationDb;
            _advancesService = advancesService;
            _BKAuditComplianceService = BKAuditComplianceService;
        }
		...
		public IActionResult _index()
		{
			try
			{
                ...
                ResultModel<List<BKAuditCompliance>> indexData =
                    _BKAuditComplianceService.GetIndexData(index, conditionalFields, conditionalValue);

				ResultModel<int> indexDataCount =
                _BKAuditComplianceService.GetIndexDataCount(index, conditionalFields, conditionalValue);

				int result = _BKAuditComplianceService.GetCount(TableName.BKAuditCompliance, "Id", new[] { "BKAuditCompliances.createdBy", }, new[] { userName });
        ...
        public ActionResult Create()
        {
            ...
            ModelState.Clear();
            BKAuditCompliance vm = new BKAuditCompliance();
            vm.Operation = "add";
            return View("CreateEdit", vm);


        }
        public ActionResult CreateEdit(BKAuditCompliance master)
        {
            ResultModel<BKAuditCompliance> result = new ResultModel<BKAuditCompliance>();
            try
            {
                ...
                if (master.Operation == "update")
                {
                    ...
                    result = _BKAuditComplianceService.Update(master);
                ...
                else
                {
                    ...
                    result = _BKAuditComplianceService.Insert(master);
		...
		public ActionResult Edit(int id)
		{
			try
            {
                ResultModel<List<BKAuditCompliance>> result =
                    _BKAuditComplianceService.GetAll(new[] { "Id" }, new[] { id.ToString() });

                BKAuditCompliance BKAuditCompliance = result.Data.FirstOrDefault();
                BKAuditCompliance.Operation = "update";
                BKAuditCompliance.Id = id;

                return View("CreateEdit", BKAuditCompliance);
            }
            catch (Exception e)
            {
                e.LogAsync(ControllerContext.HttpContext);
