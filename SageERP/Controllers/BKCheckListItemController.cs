...
namespace SSLAudit.Controllers
{
	...
	[Authorize]
	public class BKCheckListItemController : Controller
    {
        private readonly ApplicationDbContext _applicationDb;
        private readonly IAdvancesService _advancesService;
        private readonly ICheckListItemService _checkListItemService;

        public BKCheckListItemController(ApplicationDbContext applicationDb, ITeamsService teamsService,
		IAdvancesService advancesService, ICheckListItemService checkListItemService
		...
		public IActionResult _index()
		{
			try
			{
                ...
                ResultModel<List<CheckListItem>> indexData =
                    _checkListItemService.GetIndexData(index, conditionalFields, conditionalValue);

				ResultModel<int> indexDataCount =
                _checkListItemService.GetIndexDataCount(index, conditionalFields, conditionalValue);

				int result = _checkListItemService.GetCount(TableName.BKCheckListItems, "Id", new[] { "BKCheckListItems.createdBy", }, new[] { userName });
        ...
        public ActionResult Create()
        {
            ...
            ModelState.Clear();
            CheckListItem vm = new CheckListItem();
            vm.Operation = "add";
            return View("CreateEdit", vm);


        }
        public ActionResult CreateEdit(CheckListItem master)
        {
            ResultModel<CheckListItem> result = new ResultModel<CheckListItem>();
            try
            {
                ...
                if (master.Operation == "update")
                {
                    ...
                    result = _checkListItemService.Update(master);
                ...
                else
                {
                    ...
                    result = _checkListItemService.Insert(master);
		...
		public ActionResult Edit(int id)
		{
			try
            {
                ResultModel<List<CheckListItem>> result =
                    _checkListItemService.GetAll(new[] { "Id" }, new[] { id.ToString() });

                CheckListItem CheckListItem = result.Data.FirstOrDefault();
                CheckListItem.Operation = "update";
                CheckListItem.Id = id;

                return View("CreateEdit", CheckListItem);
            }
            catch (Exception e)
            {
                e.LogAsync(ControllerContext.HttpContext);
