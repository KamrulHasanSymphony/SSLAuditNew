var BKAuditPreferenceEvaluationController = function (CommonService, BKAuditPreferenceEvaluationService) {
    ...
    var init = function () {
        ...
        if ($("#BKAuditPreferenceEvaluation_BKAuditOfficeId").length) {
            //LoadCombo("BKAuditOfficeId", '/Common/BKAuditOffice');
            LoadCombo("BKAuditPreferenceEvaluation_BKAuditOfficeId", '/Common/BKAuditOffice');
        } 
        ...
        var indexTable = BKAuditPreferenceEvaluationTable();


        //$(".btnsave").click(function () {
        $(".btnsaveAuditPreferenceEvaluation").click(function () {
            save();
        });     
    ...
    function save() {
        ...
        if (bid == 0) {
            ShowNotification(3, "Please Save Branch First");
            return;
        }


        var validator = $("#frm_BKAuditPreferenceEvaluation").validate();
        var bkAuditPreferenceEvaluation = serializeInputs("frm_BKAuditPreferenceEvaluation");
        var result = validator.form();

        var isChecked = $("#Status").is(":checked");
        bkAuditPreferenceEvaluation.Status = isChecked;
        bkAuditPreferenceEvaluation["BKAuditPreferenceEvaluation.Status"] = isChecked.toString();
        ...
        BKAuditPreferenceEvaluationService.save(bkAuditPreferenceEvaluation, saveDone, saveFail);
    ...
    function saveDone(result) {
        ...
        if (result.status == "200") {
            if (result.data.operation == "add") {
                ...
                ShowNotification(1, result.message);
                //$(".btnsave").html('Update');
                $(".btnsaveAuditPreferenceEvaluation").html('Update');
                //$("#Id").val(result.data.id);
                $("#BKAuditPreferenceEvaluation_Id").val(result.data.id);
                //$("#Code").val(result.data.code);            
                $("#BKAuditPreferenceEvaluation_Code").val(result.data.code);            
                result.data.operation = "update";
                //$("#Operation").val(result.data.operation);
                $("#BKAuditPreferenceEvaluation_Operation").val(result.data.operation);
    ...
    function BKAuditPreferenceEvaluationTable() {
        var dataTable = $("#BKAuditPreferenceEvaluationList").DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            serverSide: true,
            processing: true,
            ...
            ajax: {
                url: '/BKAuditPreferenceEvaluation/_index',
                type: 'POST',
            ...
            columns: [
                {
                    ...
                    render: function (data) {
                        var bid = $("#BranchID").val();
                        return "<a href='/BranchProfile/Edit/" + bid + "?commonId=" + data + "&status=evaluation#tabs-1' class='edit btn btn-primary btn-sm'>" +
    ...
    //var BKAuditPreferenceEvaluationTable = function () {

    //    $('#BKAuditPreferenceEvaluationList thead tr')
    //        .clone(true)
    //        .addClass('filters')
    //        .appendTo('#BKAuditPreferenceEvaluationList thead');
    ...
    //                    return "<a href='/BranchProfile/Edit/" + bid + "?commonId=" + data + "&status=evaluation' class='edit btn btn-primary btn-sm' >" +
    //                        "<i class='fas fa-pencil-alt' data-toggle='tooltip' title='' data-original-title='Edit'></i></a>";
    ...
    //    $("#BKAuditPreferenceEvaluationList").on("change",
    //        ".acc-filters",
    //        function () {
    //            dataTable.draw();
    //        });

    //    $("#BKAuditPreferenceEvaluationList").on("keyup",
    //        ".acc-filters",
    //        function () {
    //            dataTable.draw();
    //        });
...
}(CommonService, BKAuditPreferenceEvaluationService);