var UserProfileController = function (CommonService,UserProfileService) {
    ...
    $("#EmployeeId").on("change", function (e) {
        ...
        $.ajax({
            type: "POST",  
            url: '/UserProfile/GetUserProfileData',  
            contentType: 'application/json',  
            data: JSON.stringify(commonVM),
            
            success: function (result) {
                var data = result[0].empName;
                debugger;

                var UserName = result[0].empName.replace(/\s+/g, '');
                $("#UserName").val(UserName + "_" + result[0].code);
                $("#ProfileName").val(result[0].empName);
                $("#Email").val(result[0].personalEmail);
                $("#PhoneNumber").val(result[0].permanentPhone);
                $("#PFNo").val(0);
