var PurchaseController = function (CommonService, ToursService) {
    ...
    var init = function () {
        ...
        //if ($("#EmpNameId").length) {
        //    LoadCombo("EmpNameId", '/Common/EmpName');
        //}
    ...
    $("#ToDate,#FromDate").on("change", function () {
        ...
        var totalDays = difference / (1000 * 60 * 60 * 24) + 1;

        $("#TotalAmount").val(Amount * totalDays);
    ...
    $("#EmployeeId").on("change", function (e) {
        debugger;
        var empDropdown = document.getElementById("EmployeeId");
        ...
        $.ajax({
            type: "POST",  
            url: '/Tours/GetEmployeeAmount',  
            contentType: 'application/json',  
            data: JSON.stringify(commonVM),  
            success: function (result) {
                var data = result[0].travelAllowance;
                debugger;
                $("#Amount").val(result[0].travelAllowance);
    ...
    function save() {
        ...
        debugger;
        var empDropdown = document.getElementById("EmployeeId");
        var EmpNameId = empDropdown.value;
        var selectedIndex = empDropdown.selectedIndex;
        var EmpName = empDropdown.options[selectedIndex].text;

        var validator = $("#frm_Tours").validate();
        var advances = serializeInputs("frm_Tours");

        advances.EmpName = EmpName;
    ...
    var ToursTable = function () {
        ...
        $('#PurchaseList thead tr')
            .clone(true)
            .addClass('filters')
            .appendTo('#PurchaseList thead');


        var dataTable = $("#PurchaseList").DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            serverSide: true,
            "processing": true,
            ...
            ajax: {
                url: '/BranchAccounting/_indexPurchase/',
                type: 'POST',
