var DeshboardEntryController = function (CommonService, DeshboardEntryService) {
    ...
    var init = function () {
        ...
        var DeshboardSettingIndexTable = DeshboardSettingTable();
        ...
        $(".btnDeshboardSettingSave").click(function () {
            deshboardSettingSave();
        });
    ...
    function deshboardSettingSave() {
        debugger;
        var validator = $("#frm_DeshboardSetting").validate();

        var deshboardSetting = serializeInputs("frm_DeshboardSetting");

        var IsPieChart = $("#isPieChart").is(":checked");
        var IsFinancialData = $("#isFinancialData").is(":checked");
        var IsPlannedEngagement = $("#isPlannedEngagement").is(":checked");
        var IsUnplannedEngagement = $("#isUnplannedEngagement").is(":checked");
        var IsYearData = $("#isYearData").is(":checked");
        var IsBoxData = $("#isBoxData").is(":checked");

        deshboardSetting.IsPieChart = IsPieChart;
        deshboardSetting.IsFinancialData = IsFinancialData;
        deshboardSetting.IsPlannedEngagement = IsPlannedEngagement;
        deshboardSetting.IsUnplannedEngagement = IsUnplannedEngagement;
        deshboardSetting.IsYearData = IsYearData;
        deshboardSetting.IsBoxData = IsBoxData;
        ...
        DeshboardEntryService.deshboardSettingSave(deshboardSetting, DeshboardSaveDone, deshboardSaveFail);

    }

    function DeshboardSaveDone(result) {
        debugger
        if (result.status == "200") {
            if (result.data.operation == "add") {

                ShowNotification(1, result.message);
                $(".btnDeshboardSettingSave").html('Update');
                //$(".btnDeshboardSettingSave").addClass('sslUpdate');
    ...
    function deshboardSaveFail(result) {
        console.log(result);
    ...
    var DeshboardSettingTable = function () {

        $('#DeshboardSettingList thead tr')
            .clone(true)
            .addClass('filters')
            .appendTo('#DeshboardSettingList thead');


        var dataTable = $("#DeshboardSettingList").DataTable({
            orderCellsTop: true,
            fixedHeader: true,
            serverSide: true,
            "processing": true,
            ...
            ajax: {
                url: '/Deshboard/_deshboardsettingIndex',
                type: 'POST',
                data: function (payLoad) {
                    return $.extend({},
                        payLoad,
            ...
            columns: [
                {
                    ...
                    render: function (data) {
                        ...
                        return "<a href=/Deshboard/DeshboardSettingEdit/" + data + " class='edit btn btn-primary btn-sm' ><i class='fas fa-pencil-alt' data-toggle='tooltip' title='' data-original-title='Edit'></i></a> "                           
                            ;
                    },
                    "width": "9%",
                    "orderable": false
                ...
                ,
                {
                    data: "isPlannedEngagement",
                    name: "IsPlannedEngagement"
                }
                ,
                {
                    data: "isUnplannedEngagement",
                    name: "IsUnplannedEngagement"
        ...
        $("#DeshboardSettingList").on("change",
            ".acc-filters",
            function () {
