...
namespace Shampan.Repository.SqlServer.CheckListItems
{
	public class CheckListItemRepository : Repository, ICheckListItemRepository
    {
        private DbConfig _dbConfig;
        private SqlConnection context;
        private SqlTransaction transaction;       
        public CheckListItemRepository(SqlConnection context, SqlTransaction transaction, DbConfig dbConfig)
        ...
        public List<CheckListItem> GetAll(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
		{
            try
            {
...
where 1=1";
                ...
                List<CheckListItem> vms = dtResult.ToList<CheckListItem>();
                return vms;
		...
		public int GetCount(string tableName, string fieldName, string[] conditionalFields, string[] conditionalValue)
		{
            ...
            try
            {
                ...
                select count(BKCheckListItems.Id)FilteredCount
                from BKCheckListItems  where 1=1 ";
        ...
        public List<CheckListItem> GetIndexData(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
		{
            string sqlText = "";
            List<CheckListItem> VMs = new List<CheckListItem>();
            DataTable dt = new DataTable();
...
";
		...
		public CheckListItem Insert(CheckListItem model)
		{
			try
			{
				string sqlText = "";

				var command = CreateCommand(@" INSERT INTO BKCheckListItems(
...
,@CreatedFrom
				...
				command.Parameters.Add("@BKCheckListSubTypesId", SqlDbType.Int).Value = model.BKCheckListSubTypesId;
        ...
        public CheckListItem MultiplePost(CheckListItem objCheckListItem)
        {
            try
            {
                string sqlText = "";
                ...
                string query = @"  update CheckListItem set 
    ...
    ,IsRejected=@IsRejected 
     ...
     where  Id= @Id ";
                ...
                foreach (string ID in objCheckListItem.IDs)
                {
                    SqlCommand command = CreateCommand(query);
                    command.Parameters.Add("@IsPost", SqlDbType.NChar).Value = "Y";
                    command.Parameters.Add("@Id", SqlDbType.BigInt).Value = ID;
                    command.Parameters.Add("@PostedBy", SqlDbType.NChar).Value = string.IsNullOrEmpty(objCheckListItem.Audit.PostedBy.ToString()) ? (object)DBNull.Value : objCheckListItem.Audit.PostedBy.ToString();
                    command.Parameters.Add("@PostedOn", SqlDbType.NChar).Value = string.IsNullOrEmpty(objCheckListItem.Audit.PostedOn.ToString()) ? (object)DBNull.Value : objCheckListItem.Audit.PostedOn.ToString();
                    command.Parameters.Add("@PostedFrom", SqlDbType.NChar).Value = string.IsNullOrEmpty(objCheckListItem.Audit.PostedFrom.ToString()) ? (object)DBNull.Value : objCheckListItem.Audit.PostedFrom.ToString();
		...
		public CheckListItem MultipleUnPost(CheckListItem vm)
		{
			...
			try
			{
				...
				foreach (string ID in vm.IDs)
				{
					...
					{
						query = @"   update CheckListItem set 
 ...
 where  Id= @Id ";
     ...
     ,IsApprovedL4=@IsApprovedL4
      ...
      where  Id= @Id "
		...
		public CheckListItem Update(CheckListItem model)
		{
			try
			{
				string sqlText = "";
				...
				string query = @"  update BKCheckListItems set

 Code                                =@Code  
,BkCheckListTypesId                  =@BkCheckListTypesId  
,BKCheckListSubTypesId               =@BKCheckListSubTypesId  
