...
namespace Shampan.Repository.SqlServer.Settings
{
    public class SettingsRepository : Repository , ISettingsRepository
    {
        ...
        public DbUpdateModel SettingsDataInsert(SettingsModel vm, string settingGroup, string settingName, string settingType, string settingValue)
        {
            try
            {
                SqlCommand checkCommand = CreateCommand(" SELECT SettingName FROM Settings WHERE SettingName = '" + settingName + "' ");
                object result = checkCommand.ExecuteScalar();
                if (result != null && result.ToString() == settingName)
                {
                    return new DbUpdateModel();
                }
                else
...
,@CreatedFrom
                    ...
                    command.Parameters.Add("@SettingGroup", SqlDbType.NChar).Value = settingGroup;
                    command.Parameters.Add("@SettingName", SqlDbType.NChar).Value = settingName;
                    command.Parameters.Add("@SettingValue", SqlDbType.NChar).Value = settingValue;
                    command.Parameters.Add("@SettingType", SqlDbType.NChar).Value = settingType;
                    command.Parameters.Add("@IsActive", SqlDbType.NChar).Value = true;
                    command.Parameters.Add("@IsArchive", SqlDbType.NChar).Value = false;
                    command.Parameters.Add("@CreatedBy", SqlDbType.NChar).Value = "ERP";
                    command.Parameters.Add("@CreatedAt", SqlDbType.NChar).Value = DateTime.Now.ToString("MMM dd yyyy");
                    command.Parameters.Add("@CreatedFrom", SqlDbType.NChar).Value = "ERP";


                    var respone = Convert.ToInt32(command.ExecuteScalar());
