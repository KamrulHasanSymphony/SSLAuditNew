...
namespace Shampan.Repository.SqlServer.Audit
{
    public class AuditIssueUserRepository : Repository, IAuditIssueUserRepository
    {
        ...
        public AuditIssueUser AudiIssueUserUpdateById(AuditIssueUser model)
        {
            try
            {
                int issueUserId = model.AuditIssueUserList.FirstOrDefault().Id;
                var issue = model.AuditIssueUserList.FirstOrDefault();
...
WHERE AuditId=@Id And EmailAddress=@EmailAddress
";


                var command = CreateCommand(sql);
                //command.Parameters.AddWithValue("@Id", issueUserId);
                command.Parameters.AddWithValue("@Id", model.AuditId);
                command.Parameters.AddWithValue("@EmailAddress", issue.EmailAddress);
                command.Parameters.AddWithValue("@IsMailSend",true);
                command.Parameters.AddWithValue("@MailCount", model.MailCount);
      ...
      ,[AuditId]
      ,[UserId]
      ,(select UserName from {AuthDbConfig.AuthDB}.dbo.AspNetUsers where Id = UserId) UserName
      ,[EmailAddress]
...
WHERE 1=1
        ...
        public List<AuditIssueUser> GetAuditIssueUserById(string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
        {
            try
            {
                string sqlText = $@"
...
";
        ...
        public List<AuditIssueUser> GetAuditIssueUsersList(string UserId, int? id)
        {
            string sqlText = "";
            List<AuditIssueUser> VMs = new List<AuditIssueUser>();
            DataTable dt = new DataTable();
...
,EmailAddress
 FROM [AuditIssueUsers]
 WHERE 1=1 and AuditId = @AuditId AND UserID = @UserID
";

                SqlDataAdapter objComm = CreateAdapter(sqlText);
                objComm.SelectCommand = ApplyParameters(objComm.SelectCommand, new string[0], new string[0]);
                objComm.SelectCommand.Parameters.AddWithValue("@UserID", UserId);
      ...
      ,(select UserName from {AuthDbConfig.AuthDB}.dbo.AspNetUsers where Id = UserId) UserName
      ,[EmailAddress]
      ,ISNULL(IsMailSend,0) IsMailSend
      ,CONCAT(
        CASE 
            WHEN ISNULL(IsMailSend, 0) = 0 THEN 'NO'
            ELSE 'YES'
        END,
        '(',
        ISNULL(MailCount,0) ,')'
  ...
  WHERE 1=1 and AuditIssueId = @AuditIssueId
";
...
";
                ...
                command.Parameters.Add("@UserId", SqlDbType.NVarChar).Value =model.UserId is null ? DBNull.Value : model.UserId;
                command.Parameters.Add("@EmailAddress", SqlDbType.NVarChar).Value = model.EmailAddress;
				command.Parameters.Add("@IsMailSend", SqlDbType.Bit).Value = false;
