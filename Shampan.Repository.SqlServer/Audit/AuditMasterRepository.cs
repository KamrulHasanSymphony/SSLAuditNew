...
from A_Audits ad 
  where 1=1 
and IsApprovedL4=1 and IsCompleteIssueBranchFeedback=1 AND ad.StartDate >= @BeginingYear AND ad.StartDate <= @EndYear
and ad.id in( select id from #TempId)
...
where 1=1 AND ad.StartDate >= @BeginingYear AND ad.StartDate <= @EndYear
...
";
                ...
                //sqlText += " AND ad.StartDate >= @BeginingYear AND ad.StartDate <= @EndYear";
...
";
                    ...
                    //sqlText += " And EXISTS (" +
                    //    " SELECT 1 FROM A_AuditIssues AI  WHERE ad.Id = AI.AuditId " +
                    //    ") ";
...
";
                ...
                else if (index.Status == "Department")
                {
                    sqlText = @"
...
";
                ...
                else if (index.Status == "Subsidiary")
                {
                    sqlText = @"
...
";
                ...
                else if (index.Status == "Underwriting")
                {
                    sqlText = @"
...
";
                ...
                else if (index.Status == "Compliance")
                {
                    sqlText = @"
...
WHERE 1=1 AND AuditTypeid = 1017
";
...
";
                ...
                else if (index.Status == "Follow-Up")
                {
                    sqlText = @"
...
";
                ...
                --WHERE 1=1 
                --AND CAST(AI.ImplementationDate AS DATE) <= CAST(GETDATE() AS DATE)
...
FROM (
    SELECT 
        fb.*,
        ROW_NUMBER() OVER (
            PARTITION BY fb.AuditIssueId 
            ORDER BY fb.CreatedOn DESC
        ) AS rn
    FROM A_AuditBranchFeedbacks fb
    WHERE CAST(fb.ImplementationDate AS date) < CAST(fb.CreatedOn AS date)
) AS T
...
";
                    ...
                    sqlText += "AND A.StartDate >= @BeginingYear AND A.StartDate <= @EndYear";
...
WHERE 1=1 and ad.id in(SELECT distinct AuditId FROM AuditUsers
...
";
    ...
    ad.[Code],
    ad.[IsApprovedL4],
    ISNULL(ad.[IsCompleteIssue], '0') AS IsCompleteIssue,
    ISNULL(ad.[IsCompleteIssueTeamFeedback], '0') AS IsCompleteIssueTeamFeedback,
    ISNULL(ad.[IsCompleteIssueBranchFeedback], '0') AS IsCompleteIssueBranchFeedback,
    ad.[IsPost],
...
ad.Id IN (SELECT DISTINCT AU.AuditId FROM AuditUsers AU
WHERE AU.UserId IN (SELECT id FROM {AuthDbConfig.AuthDB}.dbo.AspNetUsers WHERE userName = @userName)
AND NOT EXISTS (SELECT 1 FROM AuditIssueUsers AIU WHERE AIU.AuditId = AU.AuditId)) OR 
ad.Id IN (
SELECT DISTINCT
AIU.AuditId 

FROM AuditIssueUsers AIU
INNER JOIN AuditUsers AU ON AIU.AuditId = AU.AuditId

WHERE AIU.UserId IN (SELECT id FROM {AuthDbConfig.AuthDB}.dbo.AspNetUsers WHERE userName = @userName)
OR AU.UserId IN (SELECT id FROM {AuthDbConfig.AuthDB}.dbo.AspNetUsers WHERE userName = @userName))
...
";
                ...
                else if (index.Status == "Issue" || index.Status == "Feedback")
                {
                    sqlText = $@"
...
";
                ...
                else if (index.Status == "BranchFeedback")
                {
                    sqlText = @"
...
";
                ...
                else if (index.Status == "TotalFollowUpAudit")
                {
                 ...
                 --FROM A_AuditIssues AI
                 --LEFT OUTER JOIN A_Audits A on AI.AuditId=A.Id
                 --WHERE 1=1 AND CAST(AI.ImplementationDate AS DATE) <= CAST(GETDATE() AS DATE)
			     --AND AI.Id NOT IN(SELECT AuditIssueId FROM A_AuditBranchFeedbacks)


SELECT  
COUNT(Distinct T.AuditId)Id
FROM (
    SELECT 
        fb.*,
        ROW_NUMBER() OVER (
...
FROM A_Audits ad
where ad.CreatedBy=@userName and ad.BFIsApprovedL4=1
";
...
";
	    ...
	    and AI.Id not in(select AuditIssueId from A_AuditBranchFeedbacks)
                ...
                //Excel
                if (index.Status == "Audit" || index.Status == "AuditApproved")
                {
                    if (!string.IsNullOrEmpty(index.FromDate) && !string.IsNullOrEmpty(index.ToDate))
                    {
                        DateTime fromDate = DateTime.Parse(index.FromDate);
...
WHERE 
    1 = 1
    AND IsApprovedL4 = 1 AND ad.StartDate >= @BeginingYear AND ad.StartDate <= @EndYear
    AND ad.Id IN (SELECT Id FROM #TempId)
";
...
select @UserId = Id from {AuthDbConfig.AuthDB}.dbo.AspNetUsers where UserName = 'erp';
...
";
        ...
        public int GetAuditStatusDataCount(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
        {
            ...
            try
            {
                sqlText = @"
SELECT 
count(A_Audits.IsPlaned) PlanCount
...
";
      ...
      ,anm.EnumValue IssueStatus
       ...
       WHERE 1=1 AND IsFeedbackForIssue = @IsFeedbackForIssue
...
select UserName,Email from {AuthDbConfig.AuthDB}.dbo.AspNetUsers au
...
where  Id= @Id ";
        ...
        public AuditMaster BranchFeedbackCompleted(AuditMaster model)
        {
...
";
	   ...
	   ,CASE
		...
		,CASE when ai.Financial=1 then 'Financial' ELSE '' END as FinancialText
		,CASE when ai.Compliance=1 then 'Compliance' ELSE '' END as ComplianceText
...
and CreatedBy not in(select aspuser.UserName from A_Audits A

left join  A_TeamMembers as teamMbr On A.TeamId=teamMbr.TeamId
left join {AuthDbConfig.AuthDB}.dbo.AspNetUsers aspuser On aspuser.Id=teamMbr.UserId where A.Id=@AuditId)
";
                //sqlText = ApplyConditions(sqlText, conditionalFields, conditionalValue);
...
from A_AuditBranchFeedbacks ABF
where AuditIssueId=@IssueId and IsReport=1 and ABF.CreatedOn <= ABF.ImplementationDate
and CreatedBy in(select aspuser.UserName from A_Audits A
...
";
        ...
        public List<UserRolls> GetUserRoles(string UserName)
        {
            ...
            string query = $@"select Id,UserName from {AuthDbConfig.AuthDB}.[dbo].[AspNetUsers] authuser where authuser.UserName=@userName ";
...
Update [A_AuditBranchFeedbacks] set 
IsReport=@isCheck
...
";
                    ...
                    var command = CreateCommand(sql);
                    command.Parameters.Add("@Id", SqlDbType.Int).Value = val[0];
                    command.Parameters.Add("@isCheck", SqlDbType.Bit).Value = val[1];
                    int rowcount = Convert.ToInt32(command.ExecuteNonQuery());
...
UPDATE {AuthDbConfig.AuthDB}.[dbo].[AspNetUsers]
SET
...
";
        ...
        public int AuditNotificationUpdate(string notification, int? id)
        {
            try
            {
                string sql = "";
...
";
                ...
                command.Parameters.Add("@IsNotification", SqlDbType.NVarChar).Value = 0;
                int rowcount = Convert.ToInt32(command.ExecuteNonQuery());
                if (rowcount <= 0)
                {
                    throw new Exception(MessageModel.UpdateFail);
        ...
        public BKAuditTemplateDetails AuditTemplateDetails(BKAuditTemplateDetails model)
        {
            try
            {
                string sqlText = "";

                var command = CreateCommand(@" INSERT INTO AuditTemplateDetails(
...
,@Status
        ...
        public bool CheckIssueStatus(string tableName, string[] conditionalFields, string[] conditionalValue)
        {
            try
            {
                bool ÌsIssue = false;
                ...
                // ToDo sql injection
                string sqlText = "select COUNT(*) AuditId  from " + tableName + " where 1=1 ";
                ...
                int count = Convert.ToInt32(dt.Rows[0]["AuditId"]);
        ...
        public bool CheckListItemsStatus(string tableName, string[] conditionalFields, string[] conditionalValue)
        {
            try
            {
                bool isItem = true;
                ...
                string sqlText = "" +
                    "" +
                    "SELECT count(ATD.BKCheckListItemId) ListItems\r\n" +
                    "FROM AuditTemplateDetails ATD\r\n" +
                    "WHERE ATD.AuditId = @AuditId" +
                    "\r\nAND NOT EXISTS " +
                    "(\r\n    SELECT 1\r\n    " +
                    "FROM A_AuditIssues AAI\r\n   " +
                    " WHERE AAI.AuditId = ATD.AuditId" +
                    "\r\n AND AAI.CheckListItemId = ATD.BKCheckListItemId\r\n) " +
                    "";
                ...
                command.Parameters.AddWithValue("@AuditId", conditionalValue[0]);

                SqlDataAdapter dataAdapter = new SqlDataAdapter(command);
                dataAdapter.Fill(dt);

                int count = Convert.ToInt32(dt.Rows[0]["ListItems"]);
