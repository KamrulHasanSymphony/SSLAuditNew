...
namespace Shampan.Repository.SqlServer.AuditFeedbackRepo
{
    public class AuditBranchFeedbackRepository : Repository, IAuditBranchFeedbackRepository
    {
	  ...
	  ,adt.TeamId
      ,isnull(adt.IsCompleteIssueBranchFeedback,0) IsCompleteIssueBranchFeedback
...
";
                ...
                if (index.IsBranch)
                {
                    var auditIssueList = string.Join(",", index.AuditIssueUsers.Select(a => a.AuditIssueId));
                    sqlText +=  "AND AuditIssueId IN (" + auditIssueList + ")";
                }
  ...
  ";
       ...
       left outer join {AuthDbConfig.AuthDB}.dbo.AspNetUsers usr on usr.Id = IU.UserId
...
";
        ...
        public AuditBranchFeedback IssueUpdate(AuditBranchFeedback model)
        {
            try
            {
                string sql = @"
    ...
    IsFeedbackForIssue = @IsFeedbackForIssue  
    ...
    ";
                ...
                var command = CreateCommand(sql);                        
                command.Parameters.Add("@Id", SqlDbType.Int).Value = model.AuditIssueId;
               
                if (model.IsBranch)
                {
                    command.Parameters.Add("@IsFeedbackForIssue", SqlDbType.Bit).Value = true;
                }
                else
                {
                    command.Parameters.Add("@IsFeedbackForIssue", SqlDbType.Bit).Value = false;
                }
        ...
        public List<AuditBranchFeedback> GetIndexDataFeedbackPreview(IndexModel index, string[] conditionalFields, string[] conditionalValue, PeramModel vm = null)
        {
            int i = 0;
            string sqlText = "";
            List<AuditBranchFeedback> VMs = new List<AuditBranchFeedback>();
      ...
      ,af.[AuditId]
      ,af.[AuditIssueId]
      ,af.[IssueDetails] BranchFeedbackDetails
      ,af.[IsPost]
      ,af.[CreatedBy]
      ,Format(af.CreatedOn ,'dd-MM-yyyy') CreatedOn
      ,af.[CreatedFrom]
